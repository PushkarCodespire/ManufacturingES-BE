...
const login = async (req, res) => {
  ...
  try {
    ...
    if (attemptRecord?.locked_until && new Date() < new Date(attemptRecord.locked_until)) {
      const remaining = Math.ceil((new Date(attemptRecord.locked_until) - new Date()) / 60000);
      return res.status(429).json({
        success: false,
